...
bookingRoutes.get(
    ...
    async (req, res) => {
        ...
        try {
            ...
            console.log(
                "error from getting paticular user booking data",
                error.message
            );
            res.json({
    ...
    async (req, res) => {
        ...
        try {
            ...
            const transporter = nodemailer.createTransport({
                service: "gmail",
                auth: {
                    user: "xxxtentacion@gmail.com",
                    pass: process.env.emailPassword,
                },
            });
            const mailOptions = {
                from: "xxxtentacion@gmail.com",
                to: `${data.userEmail}`,
            ...
            transporter.sendMail(mailOptions, (error, info) => {
                if (error) {
                    ...
                    return res
                        .status(500)
                        .json({ msg: "Error while sending conformation mail" });
                } else {
                    return res
                        .status(200)
                        .json({
                            msg: "new booking created successfully Confiramtion sent to email",
                        });
                }
            });
        } catch (error) {
        ...
        try {
            ...
            if (currentDate > specificDate) {
                ...
                if (daysDiff >= 1) {
                    ...
                    res.json({
                        msg: `booking id of ${ID} is deleted succesfully`,
                    });
                } else {
                    return res.json({
                        msg: "Our cancellation policy requires a minimum one-day notice for booking deletions.",
                    });
                }
            }
        } catch (error) {
            ...
            res.json({
                msg: "error in deleting of booking data",
                errorMsg: error.message,
            });
        }
    }
